...
def show_paths_from_xml():
    try:
        ...
        label_ping_server = tk.Label(paths_window, text=f"Servidor de Ping: {config['pingServer']}", anchor="w", wraplength=480)
        label_ping_server.pack(fill="x", padx=10, pady=5)

        # Exibir o caminho de origem
        label_source_path = tk.Label(paths_window, text=f"Caminho de Origem: {config['sourcePath']}", anchor="w", wraplength=480)
...
def create_config_window(parent_window):
    ...
    # Definindo o tamanho da janela para centralizar
    window_width = 500
    window_height = 460
    ...
    # Aplica a geometria com a posição calculada
    ...
    # Logo GDS (substitua 'logo.png' com o caminho correto da sua imagem)
    logo_image = tk.PhotoImage(file="logo.png")  # Altere o caminho conforme necessário
    logo_label = tk.Label(header_frame, image=logo_image)
    logo_label.image = logo_image  # Necessário para manter a referência da imagem
    logo_label.pack(side="top", pady=5)
    ...
    # Área para múltiplos destinos com barra de rolagem
    ...
    canvas = tk.Canvas(destinations_frame, height=100)  # Ajuste a altura da Canvas
    scroll_y = tk.Scrollbar(destinations_frame, orient="vertical", command=canvas.yview)
...
def create_main_window():
    ...
    # Label para o contador de progresso
    progress_text_label = tk.Label(root, text="0%", fg="blue", font=("Arial", 10))
    progress_text_label.pack(pady=5)

    # Status centralizado abaixo da barra de progresso (inicialmente oculto)
